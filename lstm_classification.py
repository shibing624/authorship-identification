...
import numpy as np
import pandas as pd
from keras.layers import Dense, LSTM, Dropout, GlobalMaxPooling1D, Activation
...
import cfg
from data_utils import load_pkl, save
...
# -----------------------load data set----------------------
df_all = load_pkl(cfg.data_path + 'all.pkl')
...
embedding_dims = 100
epochs = 5
SAVE_MODEL_PATH = cfg.data_path + 'lstm_multi_classification_model.h5'
...
x = LSTM(output_dim=256, dropout=0.25, recurrent_dropout=0.25, return_sequences=True)(emb)
...
save(stack_in, pred_save_path=cfg.data_path + 'lstm_single_result.csv')
feat = 'lstm'
lb = 'label'
for l in range(stack_all.shape[1]):
    df_stack['{}_{}_{}'.format(feat, lb, l)] = stack_all[:, l]
df_stack.to_csv(cfg.data_path + 'lstm_stack_20W.csv', encoding='utf8', index=None)
print(datetime.now(), 'save lstm stack done!')
